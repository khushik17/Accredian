...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-3 z-100 flex justify-center px-2 md:px-4">
      <nav
        className={`navbar-shell mx-auto flex w-full origin-top items-center justify-between px-4 transition-[transform,max-width,padding,box-shadow,background-color,backdrop-filter] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform md:px-6 ${
          scrolled ? "max-w-4xl scale-[0.965] py-2 shadow-xl" : "max-w-7xl scale-100 py-3.5 shadow-lg"
        }`}
      >
        <a
          href="#home"
          className={`navbar-brand font-bold tracking-tight transition-all duration-500 ${
        ...
        <ul className={`hidden items-center font-medium transition-all duration-500 md:flex ${scrolled ? "gap-2.5 text-[12px]" : "gap-6 text-sm"}`}>
          {navItems.map((item) => (
            <li key={item.href}>
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <a
            href="#contact"
            className={`primary-nav-button whitespace-nowrap rounded-md text-sm font-semibold text-white transition-all duration-300 ${
      ...
      {open && (
        <div className="mobile-menu-panel absolute left-2 right-2 top-full mt-2 rounded-xl border border-slate-200 bg-white p-4 shadow-lg md:hidden">
          <ul className="space-y-3 text-sm font-medium text-slate-700">
            ...
            <li>
              <a
                ...
                className="inline-block rounded-md bg-blue-600 px-4 py-2 font-semibold text-white"
              >
                Enquire Now
              </a>
            </li>
